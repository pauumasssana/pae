...
def create_segments(seed=42):
    ...
    df1.to_csv("segment1.csv", index=False)
    df2.to_csv("segment2.csv", index=False)
...
if __name__ == "__main__":
    ...
    aligned_df.to_csv("segments.csv", index=False)
    print(f"CSV creado: segments.csv (filas={len(aligned_df)})")

    # Crear un objeto vitaldb para leer los segmentos
    segment1 = vitaldb.read_csv('segment1.csv', track_names='value', exclude=None, interval=0.005)
    segment2 = vitaldb.read_csv('segment2.csv', track_names='value', exclude=None, interval=1)
...
segments = vitaldb.read_csv('segments.csv', track_names=['segment1_value', 'segment2_aligned'], exclude=None, interval=0.005)
...
probaa = "C:\\Users\\X513\\Desktop\\PAE\\VitalParser-main\\VitalParser-main\\fdrbsau47_250702_173541.vital"
probaa.to_csv('probaa.csv', [], 0.005, return_datetime=False, return_timestamp=False)